#!/bin/bash
# 拆机实验总控 v2（新环境：A=服务端 32核 10.206.0.10，B=客户端 32核 10.206.0.2）
# 在 A 上执行，通过免密 ssh 驱动 B 跑压测，结果落盘到 B /root/echo-day04/results_split_v2/
# 用法: bash split_main_v2.sh <phase>    phase: 51|52|53|54|all
SRV=10.206.0.10
CLI=10.206.0.2
PORT=9988
DIR=/root/echo-day04
RES=$DIR/results_split_v2
CLISSH="ssh -o BatchMode=yes -o ConnectTimeout=10 root@$CLI"
mkdir -p $RES

ssh_cmd() { $CLISSH "mkdir -p $RES && cd $DIR && ulimit -n 65535 && $1"; }
...
phase54() {  # CPU 利用率: 1/4/8/16 线程, 压测 ~15s 同时 pidstat(A)+mpstat(B)
